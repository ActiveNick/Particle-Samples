...
void loop()
{
    ...
    Spark.publish("WeatherIoTHub", payload);
    
    delay(5000);
    
    snprintf(payload, sizeof(payload), "{ \"sensor\":\"weather\", \"unit\":\"F\",\"location\":\"%s\",\"measure\":\"Humi\",\"org\":\"%s\",\"value\": %h,\"device\":\"%s\", \"time\": \"%s\" }", Locn, Org, f, Disp, timeNowChar);
    Spark.publish("WeatherIoTHub", payload);
